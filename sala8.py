#############################################################################################################
                                      ...
                                      #JUEGO MONEDAS Y DUENDES MULTIJUGADOR
...
import sys
import random
import time #la partida acaba en x tiempo

tiempo_de_juego = 500 #500 segundos de juego
SIZE = (750, 625)  #tamaño de la imagen

X=0
Y=1

STEP = 15 #cuanto avanza por movimiento


class Duende():
    
    def __init__(self, number):
        self.pos =[random.randint(0,SIZE[X]),SIZE[Y] - 40] #inicio en posición random
        self.index = number #que cada una lleve su indice
    ...
    def moveleft(self):
        self.pos[X] -= STEP
        if self.pos[X] < 0:
            self.pos[X] = 0   #no pasar el borde izquierdo
     
    def moveright(self):
        self.pos[X] += STEP
        if self.pos[X] > SIZE[X]:
                self.pos[X] = SIZE[X] #no pasar el borde derecho
...
class Moneda():
    ...
    def __init__(self,n):
        ...
        self.velocity = [0, self.vel_y] #velocidad eje y random
...
class Game():
    ...
    def stop(self):
        if time.time() - self.time > tiempo_de_juego :
...
def main(ip_address):
    manager = Manager()
    print ('¿Cuántos jugadores desean comenzar la partida?') #debe coincidir en sala y en player 
    n_duendes = int(input())
    try:
